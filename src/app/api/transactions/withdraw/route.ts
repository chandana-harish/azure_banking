...
export async function POST(request: NextRequest) {
  try {
    ...
    const transaction = await createTransaction(session.sub, TransactionType.WITHDRAWAL, input.amount, input.description);
    const saved = await prisma.transaction.findUniqueOrThrow({ where: { id: transaction.id } });

    return NextResponse.json({ transaction: saved });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unable to complete withdrawal.";
